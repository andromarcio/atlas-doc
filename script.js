...
function isHidden(path) {
  ...
  return parts.some(p =>
    HIDDEN_DIRS.has(p) || p.startsWith('.')
  );
}
...
function showWelcome() {
  ...
  if (inner && welcomeEl) {
    inner.innerHTML = welcomeEl.outerHTML;
  }
  renderWelcomeStats();
}
...
function showHtmlFile(filePath) {
  ...
  document.getElementById('content-inner').innerHTML = `
    ...
    <div style="margin-bottom:1.5rem">
      <h1 style="font-size:1.375rem;font-weight:700;color:var(--text-color);margin-bottom:0.375rem">🖥️ ${name}</h1>
      <div style="font-size:0.8125rem;color:var(--text-muted-color);margin-bottom:1rem">${filePath}</div>
      <div style="display:flex;gap:0.625rem;flex-wrap:wrap">
        <button class="proto-preview-btn" onclick="openProto('${filePath}', '${name}')">
          🔲 Abrir Protótipo
        </button>
        <a href="${PAGES_BASE}/${filePath}" target="_blank" class="proto-preview-btn" style="background:var(--p-primary-50);color:var(--primary-color);border-color:var(--primary-color)">
          ↗ Abrir em nova aba
        </a>
      </div>
    </div>
    <div style="background:var(--surface-card);border:1px solid var(--surface-border);border-radius:var(--border-radius-lg);overflow:hidden;box-shadow:var(--shadow-card)">
      <div style="background:var(--surface-hover);padding:0.5rem 0.875rem;border-bottom:1px solid var(--surface-border);display:flex;align-items:center;gap:0.5rem">
        ...
        <span style="font-size:0.6875rem;color:var(--text-muted-color);margin-left:0.5rem">${filePath}</span>
      </div>
      <iframe
...
async function showRawFile(filePath) {
  ...
  document.getElementById('content-inner').innerHTML = `
    ${buildBreadcrumb(filePath, false)}
    <h1 style="font-size:1.375rem;font-weight:700;margin-bottom:1rem;color:var(--text-color)">📄 ${name}</h1>
